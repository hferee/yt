...
class Ui(object):
        ...
        while True:
            ...
            if c == ord('q'): # quit
                ...
                if idx > n_per_page:
                    ...
                    idx = 0
            elif c == ord('h'): # high quality
                self._quality = 'high'  
            elif c == ord('l'): # low quality
                self._quality = 'low'  
            elif c == ord('s'): # search
    ...
    def _play_video(self, idx):
        ...
        play_url(url, self._quality, self)
...
def play_url(url, quality, self):
    self._show_message('Fetching formats')
    yt_formats = subprocess.Popen(['youtube-dl', '-Fq', url], stdout = subprocess.PIPE, stderr = subprocess.PIPE) # this takes a few seconds. an option to disable it (ie default format)?
    formats = yt_formats.stdout.read()#or readline?
    #TODO: print the content of formats (several lines) in a ncurses window
    #self._show_message() outputs an error (lines are too long? contain strange chars?)
    #self._show_message(formats)
    form = self._input('format') # asking the user for a format code
    #TODO check if the format is one of the available ones
    try:
        yt_dl = subprocess.Popen(['youtube-dl', '-f', form, '-g', url], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
    except ValueError:
        pass
    ...
    mplayer = subprocess.Popen(['omxplayer', url.decode('UTF-8').strip()], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
    mplayer.wait()
